        ...
        function addTrimLines( handle, event )
            ...
            if trimLineExpandFlag == true
                if any(strcmp('trimLines',fieldnames(model)))
                    ...
                    % DRAW TRIMLINE
                    %                 hl2 = line(hObject,'XData',newTLX,'YData',newTLY,...
                    %                     'ZData',newTLZ,'Marker','o','Color','m');
                    %                 drawnow;
                    % ADD TRIM LINES
                    tmpDataHeight = reshape(model.data(:,3),model.numSlicePoints,...
                        numSlices);
%                     tmpDataX = reshape(model.data(:,1),model.numSlicePoints,...
%                         numSlices);
%                     tmpDataY = reshape(model.data(:,2),model.numSlicePoints,...
%                         numSlices);
%                     tmpR = sqrt(tmpDataX.^2+tmpDataY.^2);
%                     tmpDataSlop = diff(tmpR,1);
