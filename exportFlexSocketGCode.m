function  exportFlexSocketGCode( hObject, gcode_file )
...
fprintf(fid,'\n');
% GCODE FOR BOTTOM OF THE MODEL
numPoints = size(interpData,1);
for idx = 1:round(numPoints*9/12) % Print first 4 layers at F 250 as done in old R&D code
...
for idx = round(numPoints*9/12)+1:round(numPoints*10/12) % Print first 4 layers at F 250 as done in old R&D code
    fprintf(fid,'\n%s%7.4f%s%7.4f%s%4.1f', 'G1X',interpData(idx,2)...
        ,'Z',interpData(idx,1),'A',interpData(idx,3));
end
...
for idx = round(numPoints*10/12)+1:round(numPoints*11/12) % Print first 4 layers at F 250 as done in old R&D code
    fprintf(fid,'\n%s%7.4f%s%7.4f%s%4.1f', 'G1X',interpData(idx,2)...
        ,'Z',interpData(idx,1),'A',interpData(idx,3));
end
...
for idx = round(numPoints*11/12)+1:numPoints
    fprintf(fid,'\n%s%7.4f%s%7.4f%s%4.1f', 'G1X',interpData(idx,2)...
        ,'Z',interpData(idx,1),'A',interpData(idx,3));
end
