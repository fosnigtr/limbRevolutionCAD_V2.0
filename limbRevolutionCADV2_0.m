...
textureButton = handle(jFrame.getTextureButton(), 'CallbackProperties');
cutButton = handle(jFrame.getCutButton(), 'CallbackProperties');
...
exportLinerButton = handle(jFrame.getExportLinerButton(), 'CallbackProperties');
exportLinerCoreandCavityButton = handle(jFrame.getExportLinerCoreandCavityButton(), 'CallbackProperties');
exportSocketButton = handle(jFrame.getExportSocketButton(), 'CallbackProperties');
exportFlexSocketButton = handle(jFrame.getExportFlexSocketButton(), 'CallbackProperties');
exportLamSocketButton = handle(jFrame.getExportLamSocketButton(), 'CallbackProperties');
calibrateButton = handle(jFrame.getCalibrateButton(), 'CallbackProperties');
printerSettings =  handle(jFrame.getPrinterSettingsButton(), 'CallbackProperties');
...
set(textureButton,'ActionPerformedCallback', @getTexture);
set(cutButton,'ActionPerformedCallback', @getCut);
...
% PRINT PROJECT NAVIGATION
set(exportLinerButton,'ActionPerformedCallback', @getExportLiner);
set(exportLinerCoreandCavityButton,'ActionPerformedCallback', @getExportLinerCoreandCavity);
set(exportSocketButton,'ActionPerformedCallback', @getExportSocket);
set(exportFlexSocketButton,'ActionPerformedCallback', @getExportFlexSocket);
set(exportLamSocketButton,'ActionPerformedCallback', @getExportLamSocket);
set(calibrateButton,'ActionPerformedCallback', @getCalibration);
set(printerSettings,'ActionPerformedCallback', @getPrinterSettings);
...
toggleCut = true;
zm = [];
model.printerSettingsChanged = false;
...
% IMPORT FILE
    function getImportFile(hJava, hEventData)
        ...
        if strcmp(model.fname(end-3:end),'.stl') == 1
            [v, f, n, c, stltitle] = stlread([model.pname, model.fname]);
            model.data = v;
            hackTheta = 4; % set angle resolution
            hackZ = 2.9034; % set angle resolution
            % RESAMPLE HEIGHT DATA
%             totZ = size(model.data,1);
%             z = 0:totZ;
%             z = repmat(z,round(totZ/hackTheta),1);
%             z = z .* hackZ;
%             z = reshape(z,numel(z),1);
            % RESAMPLE X AND Y DATA
            dTheta = diff(atan2(model.data(:,2),model.data(:,1)));
            foo = max(abs(dTheta));
            
%             while dTheta(idx) < hackTheta
%                 
%             end
            k=find(abs(atan2(model.data(:,1),model.data(:,2)))==0);
            model.numSlicePoints = 2; % 
            model.angleStep = 2 ;
            model.sliceHeights = mean(diff(model.data(:,3)));
            model.numsliceHeights = size(model.data,1);
        elseif strcmp(model.fname(end-3:end),'.aop') == 1
        % READ .AOP FILE
            readAopFile( hObject );
        end
...
% GO TO SOURCE CODE AND DOCUMENTATION
...
% CUT MODEL
    function getCut( hJava, hEventData )
        model = guidata(hObject);
        if any(strcmp('data',fieldnames(model)))
            switch toggleCut
                case true
...
% ADD TEXTURE
    function getTexture( hJava, hEventData )
        model = guidata(hObject);
        if any(strcmp('data',fieldnames(model)))
            switch toggleCylAdap
                case true
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EXPORT LINER FILE
    function getExportLiner(hJava, hEventData)
        model = guidata(hObject);
        if any(strcmp('data',fieldnames(model)))
            ...
            if strcmp(file(end-3:end),'.tap')
                ...
                exportLinerGCode( hObject, gcode_file );
...
% EXPORT LINER CORE AND CAVITY FILE
    function getExportLinerCoreandCavity(hJava, hEventData)
        % LOAD INNER MOLD
        [fn,pn,~] = uigetfile('.mat','Open INNERMOLD.mat file');
        output = open([pn,fn]);
        model = guidata(hObject);
        model.innerModel = output.model;
        % SAVE DATA
        guidata(hObject,model);
        % LOAD OUTER MOLD
        ...
        output = open([pn,fn]);
        model = guidata(hObject);
        model.outerModel = output.model;
        % SAVE DATA
        guidata(hObject,model);
        if any(strcmp('innerModel',fieldnames(model))) && any(strcmp('outerModel',fieldnames(model)))
            % GET EXPORT FILE AND PATH NAME
            [file,path] = uiputfile({'*.tap';'*.stl'},...
                'Export file','lastName_firstName_type_yyyymmdd');
            % ERROR CHECKING
            ...
            if strcmp(file(end-3:end),'.tap')
                gcode_file = [path file];
                exportLinerCoreandCavityGCode( hObject, gcode_file );
            elseif strcmp(file(end-3:end),'.stl');
                return
            end
        else
...
% EXPORT SOCKET FILE
    function getExportSocket(hJava, hEventData)
        model = guidata(hObject);
        if any(strcmp('data',fieldnames(model)))
            % GET EXPORT FILE AND PATH NAME
            [file,path] = uiputfile({'*.tap';'*.stl'},...
            ...
            if strcmp(file(end-3:end),'.tap')
                gcode_file = [path file];
                exportSocketGCode( hObject, gcode_file );
            elseif strcmp(file(end-3:end),'.stl');
                return
            end
        else
...
% EXPORT FLEX SOCKET FILE
    function getExportFlexSocket(hJava, hEventData)
        model = guidata(hObject);
        if any(strcmp('data',fieldnames(model)))
            % GET EXPORT FILE AND PATH NAME
            [file,path] = uiputfile({'*.tap';'*.stl'},...
            ...
            if strcmp(file(end-3:end),'.tap')
                gcode_file = [path file];
                exportFlexSocketGCode( hObject, gcode_file );
            elseif strcmp(file(end-3:end),'.stl');
                return
            end
        else
...
% EXPORT LAMINATED SOCKET FILE
    function getExportLamSocket(hJava, hEventData)
        model = guidata(hObject);
        if any(strcmp('data',fieldnames(model)))
            % GET EXPORT FILE AND PATH NAME
            [file,path] = uiputfile({'*.tap';'*.stl'},...
...
% PRINTER CALIBRATION
    function getCalibration( hJava, hEventData )
        [file,path,~]=uiputfile({'*.tap'},...
                    'Export calibration file','calibrationTest_yyyymmdd');
        gcode_file = [path file];
        LayerWidthCalibrationModels( hObject, gcode_file );
    end

% PRINTER SETTINGS
    function getPrinterSettings( hJava, hEventData )
        model = guidata(hObject);
        model.printerSettingsChanged = true;
        guidata(hObject,model);
        printSettings( hObject );
    end
...
%             getAdjust([],[]);
            ...
            % test
%             r = sqrt(sum(model.data(:,1:2).^2,2));
%             r = reshape(r,model.numSlicePoints,size(model.sliceHeights,1));
%             slope = diff(r,1,2);
%             distalEndRegion = round(40 / model.sliceHeightStep); % define region within 4 cm from the distal end
%             flag = sum(sum(slope(:,distalEndRegion:end) > 3.45)); % 3.45 is the printer maximum slope resolution
%             
%             if flag > 0
%                 error('Can''t print this bitch');
%                 % could automatically correct model rather than throwing
%                 % error
%             end
            
            % remove model below 4 cm if the slope is too high in the
            % distal end region
            
            % SET EXTEND TO VALUES
            modelHeight = model.data(end,end);
            modelEndHeight = model.data(1,3);
            model.guiZEnvelope = 500;
...
%             getAdjust([],[]);
            ...
            cap1 = 38.25; cap2 = 44.25; cap3 = 50.5; % starting cap radius (mm)
            cap1H = 26; cap2H = 32; cap3H = 32; % starting cap height (mm)
%             toolingSpacer = 17; % (mm)
            model.capHeight = cat(2,cap1H,cap2H,cap3H); % cap height + distal end thickness (mm)
